...
def clean_df(df,file):
    ...
    #
    df_filter['price']=df['price'].apply(lambda x:x.replace('$','').replace(',',''))
    ...
    df_filter['date']=file.split('-')[2].replace('.csv','')
    df_filter['class']=file.split('-')[1]
    return df_filter
...
def put_database(df):
    ...
    for i in tqdm(range(len(df))):
        sql="insert into listdb(rank_small,title,url,reviews_url,stars,reviews,sell,rank_big,rank_big_name,asin,brand,on_shelf,buy_limit,price,insert_date,type,date,class) values('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(df.loc[i,'rank_small'],df.loc[i,'title'],df.loc[i,'url'],df.loc[i,'reviews_url'],df.loc[i,'stars'],str(df.loc[i,'reviews']), df.loc[i,'sell'],df.loc[i,'rank_big'],df.loc[i,'rank_big_name'],df.loc[i,'asin'],df.loc[i,'brand'],df.loc[i,'on_shelf'],df.loc[i,'buy_limit'],df.loc[i,'price'],df.loc[i,'insert_date'],df.loc[i,'type'],df.loc[i,'date'],df.loc[i,'class'])
        cur.execute(sql)
    conn.commit()
    cur.close()
    conn.close()
...
def main():
    if file_list:
        for file in file_list:
            ...
            df_final.to_csv(SAVE_PATH+file.split('-')[0].replace(PATH,'')+'.csv')
            print('======存入数据库=====')
